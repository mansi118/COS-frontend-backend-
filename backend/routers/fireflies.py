...
def auto_extract_mom(
    ...
    for item in created:
        try:
            result = _send_meeting_update_to_team(item["transcript_id"], item["title"], item.get("items", 0))
            notified.append({"title": item["title"], "sent": result.get("sent", 0)})
        except Exception:
            pass
...
def _send_meeting_update_to_team(transcript_id: str, title: str, action_count: int) -> dict:
    """Send meeting summary + action items to all team members via email."""
    # Get transcript detail
    detail = get_transcript(transcript_id)
    if not detail or "error" in detail:
        return {"sent": 0, "error": "Could not fetch transcript"}

    # Build email
    sentences = detail.get("sentences", [])
    action_items = detail.get("action_items", [])
    attendees = detail.get("attendees", [])
    attendee_names = ", ".join(a.get("name") or a.get("email") or "?" for a in attendees) or "N/A"

    # Generate summary from first few sentences
    transcript_text = " ".join(s.get("text", "") for s in sentences[:30])
    summary = transcript_text[:300] + "..." if len(transcript_text) > 300 else transcript_text

    body = f"# Meeting Update — {title}\n\n"
    body += f"**Date:** {detail.get('date', 'N/A')}\n"
    body += f"**Duration:** {detail.get('duration_mins', '?')} minutes\n"
    body += f"**Attendees:** {attendee_names}\n\n"

    if summary:
        body += f"## Summary\n\n{summary}\n\n"

    if action_items:
        body += f"## Action Items ({len(action_items)})\n\n"
        for item in action_items:
            body += f"- [ ] {item}\n"
        body += "\n"

    body += "---\n\n*Auto-generated by NeuralEDGE PULSE Command Center — powered by Vexa*"
    ...
    if not team_emails:
        return {"sent": 0, "error": "No team emails found"}

    # Send to each team member
    results = []
    for email_addr in team_emails:
        r = send_email(to=email_addr, subject=subject, body=body)
        results.append({"to": email_addr, **r})
    ...
    try:
        ...
        if slack_webhook:
            import urllib.request
            slack_msg = {
                "text": f"*📋 Meeting Update — {title}*\n"
                        f"Duration: {detail.get('duration_mins', '?')}m · Attendees: {attendee_names}\n\n"
                        f"*Action Items ({len(action_items)}):*\n"
                        + "\n".join(f"• {item}" for item in action_items[:5])
                        + (f"\n_...and {len(action_items) - 5} more_" if len(action_items) > 5 else "")
            }
    ...
    return {"sent": sent, "total": len(team_emails), "results": results}


@router.post("/auto-notify")
def auto_notify_team(transcript_id: str = Query(...)):
    """Manually trigger sending meeting update to all team members."""
    # Get meeting title
    meetings_data = _vexa_api("GET", "/meetings")
    title = "Meeting"
    for m in meetings_data.get("meetings", []):
        if str(m.get("id")) == transcript_id:
            title = m.get("title") or "Meeting"
            break

    result = _send_meeting_update_to_team(transcript_id, title, 0)
    return {
